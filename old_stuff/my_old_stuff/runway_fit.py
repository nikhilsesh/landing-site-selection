...
def find_optimal_runways(region='norcoast8', 
    ...
    for idx, (center_row, center_col) in enumerate(candidate_points):
        if idx % 100 == 0:
    ...
    print(f"Runways with {runway_width}m width buffer: {with_width} ({100*with_width/len(valid_runways):.1f}%)")
    
    # Find best runway (minimum max gradient)
    best_runway = min(valid_runways, key=lambda r: r['score'])
    print(f"\nBest runway (minimum max gradient):")
    print(f"  Center: row={best_runway['center'][0]}, col={best_runway['center'][1]}")
    print(f"  Angle: {best_runway['angle']:.1f}°")
    ...
    print(f"  Std gradient: {best_runway['gradient_metrics']['std_gradient']:.2f}°")
    print(f"  Has width buffer: {best_runway['has_width_buffer']}")
    
    # Create visualization
    visualize_runways(region, landable_map, elevation, transform, 
                     valid_runways, best_runway, runway_width, pixel_size)
...
def visualize_runways(region, landable_map, elevation, transform, 
                     valid_runways, best_runway, runway_width, pixel_size):
    """
    Create visualization showing all valid runways and the best runway.
    """
    ...
    # Draw best runway in red
    best_coords = best_runway['pixel_coords']
    ...
    ax.plot(best_xs, best_ys, color='red', linewidth=3, label='Best runway')
    
    ax.set_title(f'Best Runway (Max gradient: {best_runway["gradient_metrics"]["max_abs_gradient"]:.2f}°)')
    ax.set_xlabel('Easting (m)')
    ax.set_ylabel('Northing (m)')
    ax.ticklabel_format(useOffset=False, style='plain')
    ax.legend()
    
    # 3. Elevation profile of best runway
    ax = axes[1, 0]
    ...
    # Highlight best runway
    best_coords = best_runway['pixel_coords']
    best_geo_coords = [transform * (col, row) for row, col in best_coords]
    best_xs = [c[0] for c in best_geo_coords]
    best_ys = [c[1] for c in best_geo_coords]
    ax2.plot(best_xs, best_ys, color='black', linewidth=4, alpha=0.8, label='Best runway')
...
def print_summary_statistics(valid_runways):
    """
...
if __name__ == "__main__":
    ...
    angle_step = 5       # degrees (smaller = more thorough but slower)
    spacing_meters = 50  # spacing between candidate centers
    ...
    if results is not None:
        # Print summary statistics
        print_summary_statistics(results['valid_runways'])
        
        # Save best runway as GeoTIFF
        ...
        print(f"  - {region}_runway_analysis.png")
        print(f"  - {region}_runway_gradient_map.png")
        print(f"  - {region}_best_runway.tif")
...
#         # ========================================================================
#         # STEP 3: Save best runway as GeoTIFF for GIS use
#         # ========================================================================
...
#         print(f"       - Best runway highlighted")
#         print(f"       - Elevation profile of best runway")
...
#         print(f"     → Georeferenced raster of the optimal runway")
        
#         print(f"\nBest runway location:")
